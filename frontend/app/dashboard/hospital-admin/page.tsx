  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      <div className="bg-gradient-to-r from-teal-900 via-teal-800 to-slate-900 rounded-3xl p-8 text-white shadow-xl space-y-3">
        ...
        <h1 className="text-3xl font-black">Welcome, {user?.name || 'Hospital Admin'}!</h1>
        <p className="text-xs text-teal-100">
          Logged in as: <strong className="text-white">{user?.email}</strong> (Role: HOSPITAL_ADMIN)
        </p>
      </div>
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-soft space-y-4 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Hospital Tenant RBAC Verified</h2>
        <p className="text-xs text-slate-600 max-w-lg mx-auto leading-relaxed">
          Hospital administrator authentication successful. Multi-tenant department administration, doctor onboarding, and schedule configuration will be implemented in future phases.
        </p>
      </div>
    </div>
  );
